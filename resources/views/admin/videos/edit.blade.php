    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.videos.update", [$video->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                    ...
                    @foreach($videoCategory as $key => $item)
                        <option value="{{ $item->id }}" {{ $item->id === $video->category_id ? 'selected' : '' }}>{{ $item->title_kr }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('youtube_link') ? 'is-invalid' : '' }}" type="text" name="youtube_link" id="youtube_link" value="{{ old('youtube_link', $video->youtube_link) }}">
                    ...
                    <div class="tab-pane {{ $catTab == $key_title ? 'active' : '' }}" id="tabtitle{{ $key_title }}" style="width: 100%">
                        <div class="form-group">
                            ...
                            <input
                                ...
                                value="{{ old('title_'.$item_title, $video['title_'.$item_title]) }}"
                            >
                            @if($errors->has('title'))
