...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.bannerPost.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.banner-posts.update", [$bannerPost->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('post') ? 'is-invalid' : '' }}" name="post_id" id="post_id" required>
                    @foreach($posts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('post_id') ? old('post_id') : $bannerPost->post->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('post'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sort') ? 'is-invalid' : '' }}" type="number" name="sort" id="sort" value="{{ old('sort', $bannerPost->sort) }}" step="1">
                @if($errors->has('sort'))
                ...
                <div class="col-2">
                    <div class="form-group">
                        <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                            ...
                            <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ $bannerPost->status || old('status', 0) === 1 ? 'checked' : '' }}>
                ...
                <div class="col-10">
                    <div class="form-group">
                        <div class="form-check {{ $errors->has('header_type') ? 'is-invalid' : '' }}">
                            ...
                            <input class="form-check-input" type="checkbox" name="header_type" id="header_type" value="1" {{ $bannerPost->header_type || old('header_type', 0) === 1 ? 'checked' : '' }}>
