...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pollVariant.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.poll-variants.update", [$pollVariant->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('poll') ? 'is-invalid' : '' }}" name="poll_id" id="poll_id" required>
                    @foreach($polls as $id => $entry)
                        <option value="{{ $id }}" {{ (old('poll_id') ? old('poll_id') : $pollVariant->poll->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('poll'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $pollVariant->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sort') ? 'is-invalid' : '' }}" type="text" name="sort" id="sort" value="{{ old('sort', $pollVariant->sort) }}">
                @if($errors->has('sort'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_coccect') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_coccect" id="is_coccect" value="1" {{ $pollVariant->is_coccect || old('is_coccect', 0) === 1 ? 'checked' : '' }}>
