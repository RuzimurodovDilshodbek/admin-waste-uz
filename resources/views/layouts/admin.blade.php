...
<head>
    ...
    <title>{{ trans('panel.site_title') }} — Admin Panel</title>
        ...
        <ul class="navbar-nav ml-auto align-items-center" style="gap:8px;">
            ...
            @if(count(config('panel.available_languages', [])) > 1)
                <li class="nav-item dropdown">
                    <a class="lang-dropdown-toggle nav-link" data-toggle="dropdown" href="#">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        @foreach(config('panel.available_languages') as $langLocale => $langName)
                            <a class="dropdown-item {{ app()->getLocale() == $langLocale ? 'font-weight-bold' : '' }}"
                               href="{{ url()->current() }}?change_language={{ $langLocale }}">
                                {{ strtoupper($langLocale) }}
                                <span style="color:#94a3b8; font-weight:400; margin-left:4px;">{{ $langName }}</span>
                                @if(app()->getLocale() == $langLocale)
                                    <i class="fas fa-check ml-auto" style="color:#16a34a; font-size:11px; float:right; margin-top:2px;"></i>
                                @endif
                            </a>
                        @endforeach
                    </div>
            ...
            <li class="nav-item dropdown">
                <a class="navbar-user-btn nav-link" data-toggle="dropdown" href="#" style="text-decoration:none;">
                    <div class="navbar-user-avatar">
                        {{ strtoupper(substr(auth()->user()->name ?? 'A', 0, 1)) }}
                    </div>
                    <span class="navbar-user-name d-none d-md-block">
                        {{ auth()->user()->name ?? 'Admin' }}
                    </span>
                    <i class="fas fa-chevron-down d-none d-md-inline" style="font-size:9px; color:#94a3b8;"></i>
                </a>
                <div class="dropdown-menu dropdown-menu-right">
                    <div style="padding:12px 16px 10px; border-bottom:1px solid #f1f5f9; margin-bottom:4px;">
                        <div style="font-size:13px; font-weight:700; color:#0f172a;">{{ auth()->user()->name ?? 'Admin' }}</div>
                    ...
                    @if(file_exists(app_path('Http/Controllers/Auth/ChangePasswordController.php')))
                        @can('profile_password_edit')
                            <a class="dropdown-item" href="{{ route('profile.password.edit') }}">
                                <i class="fas fa-key" style="width:16px; color:#94a3b8;"></i>
    ...
    <footer class="main-footer">
        <div class="float-right d-none d-sm-block">
            <span style="color:#94a3b8;">
                <i class="fas fa-seedling" style="color:#16a34a; margin-right:4px;"></i>
                v1.0.0
            </span>
        </div>
        <strong style="color:#64748b;">&copy; {{ date('Y') }}</strong>
        <span style="color:#94a3b8; margin-left:4px;">{{ trans('global.allRightsReserved') }}</span>
    </footer>
