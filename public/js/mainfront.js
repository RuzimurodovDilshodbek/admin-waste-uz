...
;(function ($) {
    ...
    $(function () {
        ...
        $ajaxTab.on('click', '[data-ajax-action]', function (e) {
            var $t = $(this), $content = $t.parents('[data-ajax="tab"]').siblings('[data-ajax-content="outer"]'),
                $preloader = $content.find('.preloader'), ajaxUrl = $t.attr('href'), action = $t.data('ajax-action');

            $preloader.fadeIn();

            $.post(ajaxUrl, {action: action}, function (res) {
                $("#innerListHtml").html(res)
                $preloader.fadeOut();
            });
        ...
        $ajaxForm.each(function () {
            var $form = $(this), $formStatus = $form.find('.status');

            $form.validate({
                errorPlacement: function () {
                    return true;
                }, submitHandler: function (el) {
                    var $form = $(el), formUrl = $form.attr('action'), formData = $form.serialize();
        ...
        $formMailchimpAjax.each(function () {
            $(this).validate({
                errorPlacement: function () {
                    ...
                    var $form = $(el), $formStatus = $form.children('.status'), formData = $form.serialize(),
                        formURL = $form.attr('action'),
                        $formBtnIcon = $form.find('button[type="submit"] .fa'), formIconClass = function () {
                    ...
                    $.getJSON(formURL, formData, function (res) {
                        ...
                        $formBtnIcon.toggleClass('fa-spinner fa-spin ' + "fa-check");
                    });
                }
            });
        });
        ...
        $(document).on('click', '.send-button', function (e) {
            var $t = $(this);
            let $pollWidget = $(this).parent(".poll-form");


            $pollWidget.find('.send-button').addClass('disabled').attr('disabled', 'disabled');


            let formData = $pollWidget.serialize();
            $.ajax({
                type: "POST", // Change to "GET" or other HTTP method if needed.
                url: "/api/poll-voting", // Replace with your server-side script URL.
                data: formData,
                success: function (response) {
                    // Handle the success response from the server here.
                    $pollWidget.parents(".poll--widget").html(response)
        ...
        if ($fbWidgetIframe.length) {
            $fbWidgetIframe
                .attr('src', function (i, v) {
                    var $t = $(this), parentWidth = $t.parent().width();
         ...
         * PRELOADER
         * ------------------------------------------------------------------------- */
        var $preloaderInner = $('.preloader--inner'), preloaderSnippers = function ($el, i) {
            i = {
                now: 1, max: i, result: ''
            };
        ...
        $preloaderInner.each(function () {
            var $t = $(this), preloader = $t.parent('.preloader').data('preloader');
        ...
        $megamenu.on('click', '.megamenu--filter a[data-action]', function (e) {
            var $t = $(this), $megemenuPreloader = $t.parents('.megamenu').find('.megamenu--posts .preloader'),
                ajaxUrl = $t.attr('href'), action = $t.data('action'), cat = $t.data('cat');
        ...
        var $headerNavbar = $('.header--navbar'), $headerNavbarToggle = $('.header--navbar .navbar-toggle'),
            $headerNav = $('#headerNav');
        ...
        $.fn.initMap = function () {
            ...
            if (typeof google !== 'undefined') {
                ...
                if (typeof $el.data('map-marker') !== 'undefined') {
                    var data = $el.data('map-marker'), i = 0;
         ...
         * ------------------------------------------------------------------------- */
        var $counterItem = $('.counter--item'), counterItemH = 0;
        ...
        if ($productRatingSelect.length) {
            $productRatingSelect.barrating({
                theme: 'fontawesome-stars-o', hoverState: false
            });
        }
         ...
         * CONTACT SECTION
         * ------------------------------------------------------------------------- */
        var $contactCatsLi = $('.contact--cats > .nav > li'), contactCatsLiH = 0;

        $contactCatsLi.each(function (e) {
            var $t = $(this), height = $t.outerHeight();
         ...
         * FOOTER SECTION
         * ------------------------------------------------------------------------- */
        var $footerCopyright = $('.footer--copyright'), $footerSocial = $footerCopyright.find('.social'),
        ...
        if (typeof $.cColorSwitcher !== "undefined") {
            $.cColorSwitcher({
                'switcherTitle': 'Main Colors:&lrm;', 'switcherColors': [{
    ...
    $wn.on('load', function () {
        ...
        if ($adjustRow.length) {
            $adjustRow.isotope({
                originLeft: isRTL ? false : true, layoutMode: 'fitRows'
            });
        }
        ...
        if ($headerDropdownMenuChild.length) {
            $headerDropdownMenuChild.on('mouseenter', function () {
                ...
                if (typeof $t.data('switch') === 'undefined') {
                    setTimeout(function () {
                        var $dropdown = $t.children('.dropdown-menu'), $parent = $t.parents('.dropdown'),
                            a = $t.parents('.container').width(), b = ($parent.position().left - 15) + $t.width(),
                            c = b + $dropdown.width(), e = ($parent.position().left - 15) + $parent.width(),
                            f = $t.width() + $dropdown.width(), g = '';
         ...
         * PORTFOLIO SECTION
         * ------------------------------------------------------------------------- */
        var $portfolio = $('.portfolio--section'), $portfolioItems = $portfolio.find('.portfolio--items');

        if ($portfolioItems.length) {
            $portfolioItems.isotope({
                animationEngine: 'best-available', itemSelector: '.portfolio--item'
            });
        }
    ...
    const widget = document.getElementsByClassName('list--widget');
    ...
    modeToggleDark.addEventListener('click', () => {
        ...
        if (body.classList.contains('dark-mode')) {
            ...
            document.documentElement.style.setProperty('--image-filter-brightness', 'var(--image-filter-brightness-dark)');
            localStorage.setItem('theme', 'dark');
            for (let i = 0; i < widget.length; i++) {
                let currentElement = widget[i];
                currentElement.style.backgroundColor = '#1a1a1a';
                currentElement.style.color = '#fff';
            }
        } else {
            modeToggleDark.style.display = 'block';
            modeToggleLight.style.display = 'none';
            document.documentElement.style.setProperty('--image-filter-brightness', 'var(--image-filter-brightness-light)');
            localStorage.setItem('theme', 'light');
            for (let i = 0; i < widget.length; i++) {
                let currentElement = widget[i];
                currentElement.style.backgroundColor = '#f8f8f8';
