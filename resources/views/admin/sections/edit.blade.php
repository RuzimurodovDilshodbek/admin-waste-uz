            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent') ? 'is-invalid' : '' }}" name="parent_id" id="parent_id">
                    @foreach($parents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('parent_id') ? old('parent_id') : $section->parent->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
...
{{--                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $section->slug) }}">--}}
{{--                @if($errors->has('slug'))--}}
...
{{--            </div>--}}
                    ...
                    <div class="tab-pane {{ $catTab == $key_title ? 'active' : '' }}" id="title{{ $key_title }}" style="width: 100%">
                        <div class="form-group">
                            ...
                            <input
                                ...
                                value="{{ old('title_'.$item_title, $section['title_'.$item_title]) }}"
                            >
                            @if($errors->has('title'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ $section->status || old('status', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sort') ? 'is-invalid' : '' }}" type="text" name="sort" id="sort" value="{{ old('sort', $section->sort) }}">
                @if($errors->has('sort'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" type="text" name="url" id="url" value="{{ old('url', $section->url) }}">
                @if($errors->has('url'))
