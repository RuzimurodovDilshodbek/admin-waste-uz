...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.pollVote.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.poll-votes.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('poll_variant') ? 'is-invalid' : '' }}" name="poll_variant_id" id="poll_variant_id">
                    @foreach($poll_variants as $id => $entry)
                        <option value="{{ $id }}" {{ old('poll_variant_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
