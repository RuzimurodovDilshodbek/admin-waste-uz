...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form id="quotationCreateForm" method="POST" action="{{ route("admin.quotations.update", [$quotation->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="d-flex">
                    <div class="form-group">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" id="status" {{ ($quotation->status=="1")? "checked" : "" }} value="1" name="status">
                            <label class="form-check-label" for="status">актив</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" id="status" {{ ($quotation->status=="2")? "checked" : "" }} value="2" name="status">
                        ...
                        <div class="tab-pane {{ $catTab == $key_title ? 'active' : '' }}" id="tabtitle{{ $key_title }}" style="width: 100%">
                            <div class="form-group">
                                ...
                                <input
                                    ...
                                    value="{{ old('title_'.$item_title, $quotation['title_'.$item_title]) }}"
                                >
                                @if($errors->has('title'))
                ...
                <div class="tab-content row">
                    @foreach (config('app.locales') as $key_title => $item_title)
                        <div class="tab-pane {{ $catTab == $key_title ? 'active' : '' }}" id="authorName{{ $key_title }}" style="width: 100%">
                            <div class="form-group">
                                <label for="author_name">Муаллиф исми({{$item_title === 'kr' ? 'ўз' : $item_title }})</label>                                <input
                                    class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}"
                                    type="text"
                                    name="author_name_{{ $item_title }}"
                                    id="author_name_{{ $item_title }}"
                                    value="{{ old('author_name_'.$item_title, $quotation['author_name_'.$item_title]) }}"
                                >
...
{{--                                name="author_name"--}}
{{--                                id="author_name"--}}
{{--                                value="{{ old('author_name', $quotation['author_name']) }}"--}}
{{--                            >--}}
{{--                            @if($errors->has('author_name'))--}}
{{--                                <span class="text-danger">{{ $errors->first('author_name') }}</span>--}}
{{--                            @endif--}}
